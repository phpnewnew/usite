...
<?
  ...
  if(empty($loginuser))
  {
	header("Location:".$tae_Server ."/d/index");
  }
  
  $tnick=$loginuser->getNick();
  $tnick= addslashes($tnick);
  $member = $userMemberDAO->findMmeberByTnick($tnick);
  if($member)
  {
	$username = $member->username;
	$sqlx = "select * from whiteuser where username='".$username."'";
	$q = $aimiliPDO->query($sqlx);
  ...
  $ts = $stoTypeDAO->findTypesByWhere("where parentid=0 and status=1");
...
<div style="height:30px">
	<div style="float:left;"><a href="/d/shenheok">商品列表</a></div>
	<div style="float:left;margin-left:10px;"><a class="btn1" href="/d/shenhe">待审核列表</a></div>
	<div style="clear:both"></div>
</div>

<form action="/d/shenhe" method="GET">
<div>
...
<form action="/d/shenhe" method="POST">
...
<?php
	$where ="where status=0 and source=2 ";
	if(strlen($sscatid)>0)
	...
	?>
		<tr>
			...
			<td><?php echo $model->seller?><br/><?php echo $model->contact?></td>
			...
			<td>
				...
				<?php
					...
					$pt = $stoTypeDAO->findStoTypeById($model->ptypeid);
					foreach($ts  as $t)
					{
...
<div class="pagerlist" style="margin-top:10px">
	<div class="page-bottom tp">
		<?php 
			...
			$viewpage = $page->ViewPage($totalcount,$totalpage,$pagenum,$pagesize,$param,"/d/shenhe");
			echo $viewpage;
		?>
	</div>
</div>
